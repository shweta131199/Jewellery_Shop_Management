...
class Bill extends JFrame implements ActionListener,ItemListener
{
	JComboBox cb,tcname;
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		*/	
		...
		if(e.getSource()==ok)
		{
			if(trate.getText().length()==0 || tdiscount.getText().length()==0|| tgstper.getText().length()==0)
			{
				JOptionPane.showMessageDialog(null,"All fields are required !!!","INSERT Error",JOptionPane.WARNING_MESSAGE);
			}
			else
	...
	{try
		{
			...
			if(e.getSource()==cb)
			{
				...
				sql = "select * from product where pname='" +cb.getSelectedItem()+ "'";
