...
{
  JButton insrt,dlt,udt,srch,bk,clr,show,ext;
  ImageIcon i1;
  JLabel lname,lpri,lpdt,ledt,ltyp,lid,lqnt,lcomp,pl,ltitle,limg;
  JTextField tname,tpri,tid,tqnt,tcomp,t1;
  JComboBox cb;
  JPanel pnl;
  JTextField t;
  ...
  Font f;
JTable table;
  String colHead[] = {"P-ID","P-type","p_name","QUANTITY","PRICE"};	
	Object data[][] = {};
...
product()
  ...
  rb2=new JRadioButton("By Product id");
show.setFont(new Font("Arial",Font.PLAIN,20));  
  lst=new List();
  g=new ButtonGroup();
   
   add(lname);  add(ltyp);   add(lpri);  /* add(lpdt);  add(ledt);*/   add(lid);  add(lqnt);  add(lcomp); 
   add(tname);  add(tpri);   /*add(tpdt);  add(tedt); */  add(cb);     add(tid);  add(tqnt);  add(tcomp);	add(show);
   add(ltitle);
   cb.addItem("<Select>");       cb.addItem("Necklace");
   cb.addItem("Earring");   cb.addItem("Chain");
   cb.addItem("Bracelet");    cb.addItem("Bangles");
   cb.addItem("Ring");  cb.addItem("Ankle");
   add(pnl);
 ...
 // tedt.setBounds(190,220,150,30);
  ...
  bk.setBounds(30,360,100,30);
  clr.setBounds(140,360,100,30);
  ext.setBounds(360,360,100,30);
  ...
  public void dispTab()
	{
		try
		{
			rs = stm.executeQuery("select count(*) from product");
			rs.next();
...
public void actionPerformed(ActionEvent e)
  {
    try
   {
    if(e.getSource()==insrt)
    {
      ...
      if(tname.getText().length()==0 || cb.getSelectedIndex()==0 || tpri.getText().length()==0 )
    {
  ...
  int ans=JOptionPane.showConfirmDialog(null,"Do you want to save?");
     if(ans==0)
     {
      ...
      String sql;
      int pri,pid;
      String login_name="komal";      
      pid=Integer.parseInt(tid.getText());
      pri=Integer.parseInt(tpri.getText());
      
    sql = "insert into product values("+pid+ ",'" +cb.getSelectedItem()+ "','" +tname.getText()+ "',"+tqnt.getText()+ ",'"+tcomp.getText()+ "'," +pri+ ",'"+login_name+"')";
   ...
   if(e.getSource()==dlt)
    {
       ...
       if(tname.getText().length()==0 || cb.getSelectedItem().toString().equals("<select>") || tpri.getText().length()==0)
     JOptionPane.showMessageDialog(null,"All fields are neccessary");
   else
    {
